 ...
 *
 * 왜 배우는가:
 * VS Code의 검색 결과 강조, 구글 문서의 댓글 연결 하이라이트처럼
 * "텍스트에 색을 입히되 DOM 구조는 그대로 유지해야 하는" 요구는 매우 흔하다.
 * 기존 방식(<mark> 삽입)은 DOM을 조작하므로 React 상태와 충돌하거나 성능 이슈가 생기지만,
 * 이 API는 DOM을 건드리지 않아 에디터·검색 기능의 성능과 안정성을 동시에 잡을 수 있다.
 *
